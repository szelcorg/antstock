...
//        logger.log(Level.INFO, "Adding bar with formated time: {0}", label);
    ...
    public void updateLast(double price) {
        if (lastBar != null) {
            ...
            logger.info( "Updating last bar with date/time: "+lastBar.getDateTime().getTime());
            ...
            logger.info( "Updating last bar with formatteddate/time: "+ dataSeries.get(0).getData().get(datalength - 1).getXValue());
        }
    }
